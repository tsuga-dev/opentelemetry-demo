...
function otelHeaders(traceParent, extra) {
    const endUserBaggage = sessionPerson ? `,enduser.id=${sessionPerson.id}` : ''
    return Object.assign(
        {
            baggage: `synthetic_request=true,session.id=${sessionId}${endUserBaggage}`,
...
function onStart() {
    sessionId = uuid4()
    sessionPerson = randomChoice(people)
    const span = tracer.startSpan('user_session_start', { 'user.id': sessionPerson.id })
    span.log(`Starting session ${sessionId} for user ${sessionPerson.id}`)
...
function addToCart() {
    ...
    const span = tracer.startSpan(
        'user_add_to_cart',
        { 'user.id': sessionPerson.id, 'product.id': product, quantity }
    )
    span.log(`User ${sessionPerson.id} adding ${quantity} of product ${product} to cart`)
    const h = otelHeaders(span.traceParent())
    http.get(`${BASE_URL}/api/products/${product}`, { headers: h })
    http.post(
        `${BASE_URL}/api/cart`,
        JSON.stringify({ item: { productId: product, quantity }, userId: sessionId }),
...
function checkout() {
    const span = tracer.startSpan('user_checkout_single', { 'user.id': sessionPerson.id })
    span.log(`Starting checkout for user ${sessionPerson.id}`)

    addToCart()

    http.post(
        `${BASE_URL}/api/checkout`,
        JSON.stringify(Object.assign({}, sessionPerson, { userId: sessionId })),
        { headers: otelHeaders(span.traceParent(), { 'Content-Type': 'application/json' }) }
    )
    span.log(`Checkout completed for user ${sessionPerson.id}`)
    span.end()
}

function checkoutMulti() {
    const itemCount = randomChoice([2, 3, 4])
    const span = tracer.startSpan('user_checkout_multi', { 'user.id': sessionPerson.id, 'item.count': itemCount })
    span.log(`Starting multi-item checkout for user ${sessionPerson.id}, ${itemCount} items`)
    ...
    span.log(`Multi-item checkout completed for user ${sessionPerson.id}`)
    span.end()
}
...
async function browseProductPage(page) {
    const product = randomChoice(products)
    await page.goto(`${BASE_URL}/product/${product}`, { waitUntil: 'load' })
    await page.waitForTimeout(2000)
    await flushWebVitals(page)
}

async function browseHomeAndProduct(page) {
    const product = randomChoice(products)
    await page.goto(`${BASE_URL}/`, { waitUntil: 'load' })
    await flushWebVitals(page)
    await page.goto(`${BASE_URL}/product/${product}`, { waitUntil: 'load' })
...
async function flushWebVitals(page) {
    // A hard reload unloads the document so Faro finalizes its Core Web Vitals.
...
export async function browserScenario() {
    ...
    const browserTasks = [
        ...
        { name: 'browser_add_to_cart', run: addProductToCartBrowser },
        { name: 'browser_browse_product', run: browseProductPage },
        { name: 'browser_home_to_product', run: browseHomeAndProduct },
    ]
    const task = randomChoice(browserTasks)
    const span = tracer.startSpan(task.name, { 'user.id': person.id })
    try {
        await task.run(page)
        span.log(`${task.name} completed for user ${person.id}`)
    } catch (e) {
