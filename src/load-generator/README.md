...
Each virtual user keeps a stable named identity from `people.json` for its
session. Requests carry both `session.id` and `enduser.id` baggage, while carts
remain isolated by the per-session ID.
